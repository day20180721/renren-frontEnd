        ...
        <el-card class="box-card" style="width:80%;margin:20px auto">
          <el-form ref="spuBaseForm" :model="spu" label-width="120px" :rules="spuBaseInfoRules">
            ...
            <el-form-item label="商品介绍" prop="decript">
              <multi-upload-brand-icon ref="descImage"  @onRemove="onDecriptChange" @onSuccess="onDecriptChange" ></multi-upload-brand-icon>
              <!-- <multi-upload v-model="spu.decript"></multi-upload> -->
            </el-form-item>

            <el-form-item label="商品图集" prop="images">
              <multi-upload-brand-icon ref="image"  @onRemove="onImagesChange" @onSuccess="onImagesChange" ></multi-upload-brand-icon>
              ...
              <template slot-scope="scope">
                <el-row>
                  ...
                  <el-col :span="24">
                    ...
                    <multi-upload-brand-icon ref="plusImage" listType="text"  @onRemove="onPlusImagesRemove" @onSuccess="onPlusImagesAdd" ></multi-upload-brand-icon>
                  </el-col>
                  <!-- <el-col :span="24">
...
import MultiUploadBrandIcon from "@/components/upload/MultiUploadBrandIcon";
// import PubSub from "pubsub-js";
export default {
  //import引入的组件需要注入到对象中才能使用
  components: { CategoryCascader, BrandSelect, MultiUploadBrandIcon },
  props: {},
  data() {
    return {
      catPathSub: null,
  ...
  computed: {
    allImages: function() {
      console.log("allImage",this.spu.images.concat(this.extraUploadImages));
      return this.spu.images.concat(this.extraUploadImages);
    }
  },
  //监控data中的数据变化
  watch: {
  ...
  methods: {
    ...
    onDecriptChange(value) {
      this.spu.decript = value;
      console.log(value);
    },
    onImagesChange(value) {
      this.spu.images = value;
      console.log(value);
    },
    onPlusImagesAdd(newlist) {
      //先更新資料，在更新每個sku裡面已經點選的img欄位
      //更新extraUploadImages
      this.extraUploadImages = newlist;
      //增加所有sku中的image默認欄位
      this.spu.skus.forEach((item) => {
         item.images.push({ imgUrl: "", defaultImg: 0 });
         console.log("Add",item);
      });
    },
    onPlusImagesRemove(newlist,index) {
      //更新extraUploadImages
      this.extraUploadImages = newlist;     
      //減少所有sku中的指定image默認欄位
      this.spu.skus.forEach((item) => {
    ...
    resetUploadImages(){
      this.$refs.descImage.reset();
      this.$refs.image.reset();
      this.$refs.plusImage.reset();
    }
  },
